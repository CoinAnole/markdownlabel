...
class RefactoringValidator:
    ...
    def validate_duplication_reduction(self, before: RefactoringMetrics,
                                       after: RefactoringMetrics) -> List[str]:
        ...
        if after.duplicate_functions >= before.duplicate_functions:
            issues.append(
                "Duplicate functions not reduced: {} -> {}".format(
                    before.duplicate_functions, after.duplicate_functions))

        # Check that duplicate groups decreased
        if after.duplicate_groups >= before.duplicate_groups:
            issues.append(
                "Duplicate groups not reduced: {} -> {}".format(
                    before.duplicate_groups, after.duplicate_groups))

        # Check duplication score improvement
        before_score = before.duplication_score()
        after_score = after.duplication_score()
        if after_score >= before_score:
            issues.append("Duplication score not improved: {:.1f} -> {:.1f}".format(
                before_score, after_score))

        return issues

    def validate_coverage_preservation(self, before: RefactoringMetrics,
                                       after: RefactoringMetrics) -> List[str]:
        ...
        if after.test_coverage_percent < (before.test_coverage_percent - tolerance):
            issues.append(
                "Test coverage decreased: {:.1f}% -> {:.1f}%".format(
                    before.test_coverage_percent, after.test_coverage_percent))

        # Check that test count is preserved
        if after.total_tests < before.total_tests:
            issues.append("Test count decreased: {} -> {}".format(
                before.total_tests, after.total_tests))
    ...
    def generate_detailed_report(self, before: RefactoringMetrics, after: RefactoringMetrics) -> str:
        ...
        lines = [
            ...
            "Before Refactoring:",
            "  - Duplicate functions: {}".format(before.duplicate_functions),
            "  - Duplicate groups: {}".format(before.duplicate_groups),
            "  - Files with duplicates: {}".format(before.files_with_duplicates),
            "  - Duplication score: {:.1f}".format(before.duplication_score()),
            "",
            "After Refactoring:",
            "  - Duplicate functions: {}".format(after.duplicate_functions),
            "  - Duplicate groups: {}".format(after.duplicate_groups),
            "  - Files with duplicates: {}".format(after.files_with_duplicates),
            "  - Duplication score: {:.1f}".format(after.duplication_score()),
            "",
            "Improvement:",
            "  - Duplicate functions reduced by: {} ({:.1f}%)".format(
                before.duplicate_functions - after.duplicate_functions, duplication_improvement),
            "  - Duplicate groups reduced by: {}".format(
                before.duplicate_groups - after.duplicate_groups),
            "  - Estimated lines saved: {}".format(
                before.consolidation_savings - after.consolidation_savings),
            "",
            "📈 TEST COVERAGE METRICS",
            "Before: {:.1f}%".format(before.test_coverage_percent),
            "After: {:.1f}%".format(after.test_coverage_percent),
            "Change: {:+.1f}%".format(coverage_change),
            "",
            "📋 TEST SUITE METRICS",
            "Test files: {}".format(after.total_test_files),
            "Total tests: {}".format(after.total_tests),
        ]
        return "\n".join(lines)
    ...
    def run_full_validation(self) -> ValidationReport:
        ...
        if before_metrics.duplicate_functions > 0:
            report.duplication_improvement = (
                (before_metrics.duplicate_functions - current_metrics.duplicate_functions)
                / before_metrics.duplicate_functions
            ) * 100
...
def main():
    ...
    if report.validation_passed:
        ...
        print("   - Code duplication improved by {:.1f}%".format(report.duplication_improvement))
        print("   - Test coverage changed by {:+.1f}%".format(report.coverage_change))
    else:
