    ...
    def analyze_max_examples_patterns(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'overall_statistics': {
                ...
                'average_max_examples': (
                    sum(max_examples_values) / len(max_examples_values) if max_examples_values else 0
                ),
                'median_max_examples': (
                    sorted(max_examples_values)[len(max_examples_values)//2] if max_examples_values else 0
                )
            },
        ...
        for strategy_type, files in strategy_files.items():
            ...
            strategy_performance[strategy_type] = {
                ...
                'average_time_per_test': (
                    execution_time / strategy_test_count if strategy_test_count > 0 else 0
                ),
                'return_code': result.returncode
            }
