...
if result.critical_violations:
    ...
    for violation in result.critical_violations:
        print(f'  - {{violation.file_path}}:{{violation.test_name}} ({{violation.current_examples}} → {{violation.recommended_examples}})')

if result.warnings:
    print('\\nWarnings:')
    for warning in result.warnings:
        print(f'  - {{warning.file_path}}:{{warning.test_name}} ({{warning.current_examples}} → {{warning.recommended_examples}})')
...
def fix_over_testing():
    ...
    for recommendation in result.critical_violations + result.warnings:
        ...
        if old_pattern in content:
            ...
            print(f'Fixed {{recommendation.test_name}} in {{file_path}}: {{recommendation.current_examples}} → {{recommendation.recommended_examples}}')
    
    print(f'\\nFixed {{len(result.critical_violations + result.warnings)}} tests in {{len(fixed_files)}} files')
