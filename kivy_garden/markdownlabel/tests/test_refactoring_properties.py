...
class TestTestClassOrganization:
    ...
    def _are_methods_related(self, methods):
        ...
        for method in methods:
            ...
            # Font-related themes
            if any(word in method_lower for word in [
                'font', 'size', 'family', 'name', 'height', 'line_height'
            ]):
                themes.append('font')
            ...
            if any(word in method_lower for word in [
                'rebuild', 'preserve', 'identity', 'widget_tree', 'trigger'
            ]):
            ...
            if any(word in method_lower for word in [
                'helper', 'availability', 'consolidation', 'infrastructure',
            ...
            if any(word in method_lower for word in [
                'validation', 'compliance', 'format', 'standard', 'consistency'
            ]):
                themes.append('validation')

            # Clipping/sizing themes
            if any(word in method_lower for word in [
                'clip', 'size', 'sizing', 'height', 'width', 'constrain'
            ]):
            ...
            if any(word in method_lower for word in [
                'error', 'handling', 'exception', 'malformed', 'nonexistent'
            ]):
                themes.append('error')
    ...
    def test_test_methods_are_logically_grouped(self):
        ...
        **Validates: Requirements 5.1**
        ...
        if violations:
            violation_details = '\n'.join([
                f"  - {v['file']}::{v['class']}: {v['issue']} "
                f"(methods: {', '.join(v['methods'][:3])}"
                f"{'...' if len(v['methods']) > 3 else ''})"
                for v in violations
            ])
