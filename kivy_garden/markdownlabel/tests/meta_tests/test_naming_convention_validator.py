...
from kivy_garden.markdownlabel.tests.modules.naming_convention_validator import (
    NamingConventionValidator, NamingViolationType
)
from kivy_garden.markdownlabel.tests.modules.assertion_analyzer import (
    AssertionAnalyzer
)
...
'''
        ...
        analysis = self.assertion_analyzer.analyze_test_method(
            test_method, "test_file.py"
        )
...
'''
        ...
        rebuild_violations = [
            ...
            if v.violation_type == NamingViolationType.REBUILD_MISMATCH
        ]
        value_violations = [
            v for v in violations
            if v.violation_type == NamingViolationType.VALUE_MISMATCH
        ]
...
def test_naming_pattern_consistency_property(test_name_base, assertion_type):
    ...
    assertion types) do not generate rebuild or value naming violations."""
...
'''
    ...
    try:
        ...
        analysis = assertion_analyzer.analyze_test_method(
            test_method, "test_file.py"
        )
        violations = validator.validate_test_method(analysis)

        # Tests with consistent naming should have fewer violations
        # (may still have pattern consistency violations,
        # but not rebuild/value mismatches)
        rebuild_violations = [
            v for v in violations
            if v.violation_type == NamingViolationType.REBUILD_MISMATCH
        ]
