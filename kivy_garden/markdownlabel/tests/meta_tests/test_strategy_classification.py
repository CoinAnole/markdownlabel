...
from kivy_garden.markdownlabel.tests.modules.strategy_analyzer import (
    StrategyClassifier, StrategyType
)
...
class TestMaxExamplesCalculation:
    ...
    def test_small_finite_uses_input_space_size(self, range_size):
        ...
        assert optimal == range_size, (
            f"Expected {range_size} examples for range of size {range_size}, "
            f"got {optimal}"
        )
    ...
    def test_sampled_from_uses_list_length(self, items):
        ...
        assert optimal == expected_size, (
            f"Expected {expected_size} examples for {len(items)} items, "
            f"got {optimal}"
        )
    ...
    def test_complex_strategy_uses_complexity_based_examples(self, complexity_level):
        ...
        assert optimal in possible, (
            f"Expected one of {possible} examples for complexity "
            f"{complexity_level}, got {optimal}"
        )
...
class TestCombinationStrategies:
    ...
    def test_combination_uses_product_formula(self, size1, size2):
        ...
        strategy_code = (
            ...
            f'st.integers(min_value=0, max_value={size2-1}))'
        )
        ...
        assert optimal in possible, (
            f"Expected one of {possible} examples for combination "
            f"{size1}×{size2}, got {optimal}"
        )
    ...
    def test_large_combination_capped_at_fifty(self, large_size):
        ...
        strategy_code = (
            f'st.tuples(st.integers(min_value=0, max_value={large_size-1}), '
            f'st.integers(min_value=0, max_value={large_size-1}))'
        )
        ...
        assert optimal in possible, (
            f"Expected one of {possible} examples for size {large_size}, "
            f"got {optimal}"
        )
    ...
    def test_combination_with_infinite_strategy(self):
        ...
        assert 10 <= optimal <= 50, (
            f"Combination with infinite component should use 10-50 examples, "
            f"got {optimal}"
        )
...
class TestOverTestingDetection:
    ...
    def test_boolean_over_testing_detected(self, excessive_examples):
        ...
        is_over_testing = self.analyzer.calculator.is_over_testing(
            strategy_code, excessive_examples
        )

        assert is_over_testing, (
            f"Should detect over-testing for boolean with {excessive_examples} examples"
        )
    ...
    def test_small_finite_over_testing_detected(self, range_size, excessive_examples):
        ...
        if excessive_examples > optimal:
            ...
            assert is_over_testing, (
                f"Should detect over-testing for range size {range_size} with "
                f"{excessive_examples} examples"
            )
    ...
    def test_appropriate_examples_not_flagged(self):
        ...
        for strategy_code, appropriate_examples in test_cases:
            is_over_testing = self.analyzer.calculator.is_over_testing(
                strategy_code, appropriate_examples
            )
            assert not is_over_testing, (
                f"Should not flag {strategy_code} with {appropriate_examples} "
                f"examples as over-testing"
            )
...
'''
        ...
        try:
            ...
            assert analysis.total_tests == 2, (
                f"Should find 2 tests, found {analysis.total_tests}"
            )
            assert analysis.over_tested_count == 2, (
                f"Should find 2 over-tested, found {analysis.over_tested_count}"
            )

            # Check specific recommendations
            boolean_rec = next(
                (r for r in analysis.recommendations if 'boolean' in r.test_name),
                None
            )
            assert boolean_rec is not None, "Should have recommendation for boolean test"
            assert boolean_rec.recommended_examples == 2, (
                f"Boolean should recommend 2 examples, got "
                f"{boolean_rec.recommended_examples}"
            )

            range_rec = next(
                (r for r in analysis.recommendations if 'range' in r.test_name),
                None
            )
            assert range_rec is not None, "Should have recommendation for range test"
            assert range_rec.recommended_examples == 3, (
                f"Range should recommend 3 examples, got "
                f"{range_rec.recommended_examples}"
            )
