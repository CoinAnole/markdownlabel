...
class AssertionAnalyzer:
    ...
    def __init__(self):
        ...
        self.compiled_rebuild_patterns = [
            re.compile(p, re.IGNORECASE) for p in self.rebuild_patterns
        ]
        self.compiled_value_patterns = [
            re.compile(p, re.IGNORECASE) for p in self.value_patterns
        ]
        self.compiled_existence_patterns = [
            re.compile(p, re.IGNORECASE) for p in self.existence_patterns
        ]
        self.compiled_exception_patterns = [
            re.compile(p, re.IGNORECASE) for p in self.exception_patterns
        ]

    def analyze_test_method(
        self,
        test_method_node: ast.FunctionDef,
        file_path: str
    ) -> AssertionAnalysis:
        ...
        # Classify assertions
        has_rebuild = any(
            a.assertion_type == AssertionType.REBUILD for a in assertions
        )
        has_value = any(
            a.assertion_type == AssertionType.VALUE_CHANGE for a in assertions
        )
        ...
        naming_mismatch = self._detect_naming_mismatch(
            test_method_node.name, primary_type, has_rebuild
        )
        suggested_name = self._suggest_name_pattern(
            test_method_node.name, primary_type, has_rebuild
        )
    ...
    def _analyze_assert_node(
        self,
        node: ast.Assert
    ) -> Optional[AssertionPattern]:
        ...
        try:
            ...
            assertion_type, confidence, details = self._classify_assertion_code(
                code
            )
    ...
    def _analyze_call_node(
        ...
        try:
            ...
            if any(pattern in code.lower() for pattern in [
                'assert', 'equal', 'true', 'false', 'none', 'raises'
            ]):
                assertion_type, confidence, details = (
                    self._classify_assertion_code(code)
                )
    ...
    def _determine_primary_assertion_type(
        self,
        assertions: List[AssertionPattern]
    ) -> AssertionType:
...
def main():
    ...
    if path.is_file():
        ...
        for test_file in path.glob('test_*.py'):
            ...
            mismatches += sum(
                1 for a in analyses if a.naming_mismatch_detected
            )
            ...
            if analyses:
                ...
                for analysis in analyses:
                    if analysis.naming_mismatch_detected:
                        print(
                            f"  MISMATCH: {analysis.test_name} -> "
                            f"{analysis.suggested_name_pattern}"
                        )

        print(
            f"\nSummary: {total_analyses} tests analyzed, "
            f"{mismatches} naming mismatches found"
        )
