...
class OptimizationDetector:
    ...
    def get_optimization_summary(self, test_code: str, max_examples: int) -> Dict[str, any]:
        ...
        if opt_info.base_examples and opt_info.reduced_examples:
            improvement = (
                (opt_info.base_examples - opt_info.reduced_examples) /
                opt_info.base_examples
            ) * 100
        else:
            improvement = None
...
class OptimizationAwareCommentStandardizer:
    ...
    def generate_comment_with_optimization_awareness(self, test_code: str, max_examples: int) -> str:
        ...
        strategy_value = strategy_classification.strategy_type.value
        rationale = strategy_classification.rationale
        return f"# {strategy_value} strategy: {max_examples} examples ({rationale})"
                    ...
                    if opt_info:
                        if opt_info.ci_specific:
                            results['ci_optimized_tests'].append({
                                **test_entry,
                                'optimization_type': (
                                    opt_info.ci_pattern_type.value
                                    if opt_info.ci_pattern_type else None
                                ),
                                'base_examples': opt_info.base_examples,
    ...
    def analyze_optimization_coverage(self, test_files: List[str]) -> Dict[str, any]:
        ...
        if performance_improvements:
            avg_improvement = (
                sum(performance_improvements) /
                len(performance_improvements)
            )
        else:
            avg_improvement = 0
