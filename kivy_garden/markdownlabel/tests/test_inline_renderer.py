...
class TestURLMarkupSafetyProperty:
    ...
    def test_urls_with_brackets_are_safe(self, full_url):
        ...
        assert ref_opens == 1, \
            f"Should have exactly 1 [ref= tag, got {ref_opens}. URL: {full_url!r}, Result: {result!r}"
        assert ref_closes == 1, \
            f"Should have exactly 1 [/ref] tag, got {ref_closes}. URL: {full_url!r}, Result: {result!r}"
        ...
        assert '[' not in escaped_url, \
            f"Escaped URL should not contain literal [. URL: {full_url!r}, Escaped: {escaped_url!r}"
        assert ']' not in escaped_url, \
            f"Escaped URL should not contain literal ]. URL: {full_url!r}, Escaped: {escaped_url!r}"
